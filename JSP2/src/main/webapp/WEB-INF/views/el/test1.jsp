...
<body>
	...
	<pre>
		...
		- \${paramValues.key} : key 일치하는 파라미터들(배열) 모두 얻어오기
		- \${param.key[index]} : key 일치하는 파라미터들 중 index 번째 얻어오기
		- \${paramValues.key[index]} : key 일치하는 파라미터들(배열) 중 index 번째 얻어오기
	</pre>
	<hr>
	...
	<ul>
		<%-- check가 여러 개 전달된 경우 첫번째 파라미터값 출력 --%>
		<li>param.check : ${param.check}</li>
		
		<%-- check가 여러 개 전달된 경우 String[]로 반환 --%>
